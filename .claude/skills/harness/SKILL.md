...
**단일 스킬 하네스 (권장 · 소규모 서브 에이전트 하네스):** 에이전트가 2개이고 서브 에이전트 모드인 생성-검증 하네스는 별도 `*-orchestrator` 스킬을 만들지 않고 **주 스킬 한 파일**에 `## 기준`(또는 `## 리뷰 루브릭`, 에이전트 공용 규칙)과 `## 오케스트레이션`(메인 스레드가 두 에이전트를 굴리는 절차) 섹션을 함께 둔다. 주 스킬 `description`은 사용자 대면 트리거를 담고, 에이전트는 주 스킬을 경로로 참조하며 자기 섹션만 읽는다. 파일이 하나라 기준과 절차가 어긋날 일이 없고 감사가 단순해진다. 팀 모드거나 에이전트 3개 이상이면 아래 모드별 패턴대로 오케스트레이터를 분리한다.
